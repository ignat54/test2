...
form_data['gender'] = st.radio("1) Пол:", [(
    "", -1), ("Мужчина", 1), ("Женщина", 0)], format_func=lambda x: x[0], index=0)[1]
form_data['age'] = st.radio("2) Возраст:", [(
    "", -1), ("Больше 40 лет", 1), ("Меньше 40 лет", 0)], format_func=lambda x: x[0], index=0)[1]
form_data['smoking'] = st.radio("3) Курение:", [(
    "", -1), ("Да", 1), ("Нет", 0)], format_func=lambda x: x[0], index=0)[1]
form_data['waist'] = st.radio("4) Ваш обхват талии в норме? (Для мужчин < 94, для женщин < 80):", [
                              ("", -1), ("Да", 0), ("Нет", 1)], format_func=lambda x: x[0], index=0)[1]
form_data['familyHistory'] = st.radio("5) Имеются ли ССЗ у ближайших родственников:", [
                                      ("", -1), ("Да", 1), ("Нет", 0)], format_func=lambda x: x[0], index=0)[1]
form_data['cholesterol'] = st.radio("6) Уровень холестерина:", [("", -1), ("Менее 3,0", 0), ("3,0-3,9", 1),
...
form_data['bloodPressure'] = st.radio("7) Систолическое артериальное давление (верхнее):", [(
    "", -1), ("Менее 140", 0), ("140-159", 1), ("Более 160", 2), ("Неизвестно", 1)], format_func=lambda x: x[0], index=0)[1]
...
if st.button("Рассчитать риск"):
    if -1 in form_data.values():
        ...
        if total_score <= 3:
            st.success("Продолжайте вести здоровый образ жизни!")
        elif total_score <= 9:
            st.warning(
                "Вам стоит пересмотреть свой образ жизни и **планово обратиться за консультацией к врачу-терапевту**.")
        else:
            st.error(
                "Вам следует **как можно скорее обратиться за консультацией к врачу-терапевту!**")
